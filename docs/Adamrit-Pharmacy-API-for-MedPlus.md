# Adamrit Pharmacy API — Integration Guide for MedPlus

**Prepared for MedPlus by Adamrit (Hope Multi-Specialty Hospital).**

This document is written for the engineer or coding agent building the MedPlus
side of this integration. It is self-contained — everything needed to build
against the API is here. Follow it in order.

Every endpoint, field name and status code has been checked against the live
implementation. Do not guess field names, and do not invent parameters that are
not listed. If something here does not match what the API actually returns, that
is our bug — tell us, with the `requestId` from the response.

**Your API key and sync secret are sent separately.** They are not in this
document.
...
The `items` in a pushed page are the same shape as `/stock` rows (§11), and — as
with the pull snapshot — contain only orderable stock, so there are no
zero-quantity rows to process.
...
Contact Adamrit with:

- The `requestId` from any failing response (it is in the body and the
  `X-Request-Id` header) — we can look up the exact request at our end.
- Your inbound HTTPS endpoint, if you want the nightly push (§7 Option B). Not
  required to go live.

Please tell us before you go live, and again before any significant increase in
your polling rate or order volume.
