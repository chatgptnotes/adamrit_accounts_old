...
/**
 * Opening cash, on the tile.
 *
 * This already existed on the desktop Cash Handover page and nowhere else, so
 * the person actually holding the drawer -- who works from the tablet -- could
 * not record it. Same RPC, so a figure recorded here and one recorded on the
 * desktop are the same record.
 *
 ...
 * read low. The two are entered separately because they are counted separately
 * and in different places, then recorded as one opening total -- with the split
 * kept in the note, so a figure can be explained later without anybody having
 * to remember it.
 */
function OpeningCashCard({
  ...
  const record = useMutation({
    ...
    onSuccess: () => {
      ...
      setDrawer(""); setLocker(""); setNote(""); setOpen(false);
    },
    ...
    <TabletCard className="space-y-3 border-slate-300">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Count everything the counter physically holds now — the drawer and the locker
          — and record it. The system started counting part-way through a day, so cash
          already there was never seen. Recorded once and carried into the next handover.
        </p>
      </div>

      <div>
        <TabletLabel htmlFor="oc-drawer">Cash in the drawer</TabletLabel>
        <TabletInput
          id="oc-drawer"
          inputMode="decimal"
          value={drawer}
