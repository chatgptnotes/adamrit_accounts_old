  ...
  if (cases.length === 0) {
    return (
      <p className="py-10 text-center text-sm text-muted-foreground">
        Search to choose a case for implant invoice.
      </p>
    );
  }
      ...
      {cases.map((item) => (
        ...
        >
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {item.packageName || "No package"} | Registration {item.registrationId || item.yojanaRegistrationId || "N/A"}
          </p>
        </button>
      ))}
    </div>
...
export default function ImplantBillFlow() {
  ...
  const defaultBillDate = useMemo(() => formatDateInput(selected?.admissionDate || selected?.dischargeDate || null), [selected]);

  return (
    <FlowScaffold
      heading="Implant Invoice"
      subheading="Create the invoice from tablet/mobile"
      actions={
        selected ? (
          ...
          <div className="overflow-x-auto rounded-2xl border border-border bg-card p-2">
            <div className="min-w-[860px]">
              <ImplantBillSection
                ...
                defaultImplantName={selected.packageName || ""}
              />
            </div>
          </div>
        </TabletCard>
