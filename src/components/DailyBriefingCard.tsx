  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from('vouchers')
        .select('total_amount, is_auto, voucher_types(voucher_category)')
        .eq('voucher_date', today)
        .neq('status', 'CANCELLED')
        .limit(3000);
      if (error) throw error;
      const rows = (data || []) as { total_amount: number; is_auto: boolean; voucher_types: { voucher_category: string } | null }[];
