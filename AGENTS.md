...
- Tablet HR tile integrates Adamrit-side HRPulse data through `staff_attendance`, `staff_members`, `hr_leave_requests`, and `hr_payroll_slips`; employee views must be self-scoped, while SuperAdmin/Admin/HR get the master dashboard, late attendance alerts, leave approvals, and payroll verification.
