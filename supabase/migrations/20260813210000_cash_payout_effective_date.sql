...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  cash_ledgers AS (
    -- Matched on the GROUP, not the name. The merge renamed the stub to
    -- "Cash in Hand (merged 02 Aug 26)", so a name test excluded it -- and
    -- since Hope's payments still post there, Hope's whole drawer vanished.
    -- Both spellings of the group are in use across the cutover.
  ...
  joined AS (
    SELECT m.voucher_id, m.amount, m.at, m.entry_date,
           ...
           pv.cash_handover_id,
           -- When the cash actually left the counter. A payment filed in July
           -- and posted to the ledger today left the drawer in July; dating it
           -- by the posting would charge it to tonight's cashier.
