-- Credit a pharmacy sale to Medicine Sales, not to Hospital Services Income.
--
-- Hope Pharmacy's Day Book for 27 July shows the shape of it:
--
--   PUR-000008  Medicine Purchase          Purchase   5.25 Dr
--   REC-012489  Hospital Services Income   Receipt   20.00 Cr
--
-- The purchase is right. The sale is not. Every receipt from
-- patient_payment_transactions credited 4160 Hospital Services Income
-- whatever it was for, so a strip of paracetamol sold over the counter read
-- as hospital services revenue.
--
-- Two consequences. 4150 Medicine Sales stayed empty while medicine purchases
-- accumulated against it, which makes the pharmacy look like it buys stock and
-- sells nothing. And hospital services income was overstated by every pharmacy
-- sale, so neither figure could be trusted on its own.
--
-- Now the head follows payment_source: PHARMACY and DIRECT_SALE credit 4150,
-- everything else keeps 4160, with 4160 as the fallback if 4150 is missing.
--
-- NOT CHANGED, AND WORTH A SEPARATE LOOK. OPD_SERVICE and PHYSIOTHERAPY still
-- credit 4160. Consultation and physiotherapy have their own heads, 4110 and
-- 4120, and physiotherapy bills already accrue to 4120 on their own - so a
-- physiotherapy payment may be counted twice, once by the bill and once by the
-- receipt. That needs measuring before it is touched, not guessing.
--
