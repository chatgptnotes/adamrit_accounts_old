...
--
-- 20260804150000 resolved the manager's creditor ledger in M.L. Enterprises'
-- books by name and created it on first use, so "a new manager never blocks an
-- approval". The split-RM rewrite in 20260805121000 dropped that and instead
-- demanded that the allocation's rm_ledger_account_id already point at an
-- active ml_enterprises ledger.
--
-- Nothing ever repointed it. relationship_managers.ledger_account_id holds the
-- manager's hospital-side creditor ledger (the RM_ / DRMFRESH_ series under
-- DRM Hope), which is what the Relationship Manager master and the legacy
-- rm-cut payment flow both read — it is deliberately not an M.L. ledger. And
-- 20260730180000 made the RM_ series company-neutral, so company_id IS NULL
-- there, which fails 'IS DISTINCT FROM v_ml_company' as well.
--
-- So the guard could never pass. Restore the resolve-or-create: the commission
-- leg credits the manager's ledger in M.L. Enterprises' books, looked up by
-- name and created (MLRM_ series) when this manager has not been paid from
-- those books before. rm_ledger_account_id stays as it is — it remains the
-- hospital-side reference the other flows depend on.
...
DECLARE
  ...
  v_alloc_count INT; v_order INT := 1; v_rm_ml_ledger UUID;
BEGIN
  ...
  END IF;
  -- The M.L. ledger below is keyed on the manager's name, so it has to be there.
  IF EXISTS(SELECT 1 FROM daily_revenue_rm_allocations WHERE revenue_entry_id=p_entry_id AND btrim(COALESCE(rm_name,''))='') THEN
    RAISE EXCEPTION 'Every relationship manager must be named before approval';
  END IF;
  ...
  FOR v_alloc IN SELECT * FROM daily_revenue_rm_allocations WHERE revenue_entry_id=p_entry_id ORDER BY position LOOP
    ...
    SELECT id INTO v_rm_ml_ledger FROM chart_of_accounts
     WHERE company_id=v_ml_company AND is_active AND lower(btrim(account_name))=lower(btrim(v_alloc.rm_name)) LIMIT 1;
    IF v_rm_ml_ledger IS NULL THEN
      INSERT INTO chart_of_accounts(account_code,account_name,account_type,account_group,company_id,opening_balance,opening_balance_type,is_active)
      VALUES('MLRM_'||lpad(nextval('public.ml_rm_ledger_seq')::TEXT,6,'0'),btrim(v_alloc.rm_name),'CURRENT_LIABILITIES','Sundry Creditors',v_ml_company,0,'CR',true)
      RETURNING id INTO v_rm_ml_ledger;
    END IF;
    v_order:=v_order+1;
    INSERT INTO voucher_entries(id,voucher_id,account_id,narration,debit_amount,credit_amount,entry_order,bill_ref,created_at)
    VALUES(gen_random_uuid(),v_voucher_id,v_rm_ml_ledger,'Referral commission '||v_alloc.rm_name||' — '||COALESCE(v_entry.patient_name,'patient'),0,v_alloc.allocated_cut,v_order,btrim(p_bill_number),now());
  END LOOP;
