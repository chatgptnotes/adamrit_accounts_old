-- Send Hope patients to the right legal entity.
--
-- Both the patient-ledger trigger and the bill accrual resolved the company as
-- `ayushman -> ayushman_nagpur, else -> hope_partnership`. But Hope Hospitals
-- (partnership) is the LEGACY entity. The Tekhanaka hospital, which trades
-- under the brand name Hope Hospital, now operates as DRM Hope Hospital
-- Private Limited, and that is the entity whose Tally opening balances were
-- imported at the cutover.
--
-- Result: 1,730 patient ledgers sat under the partnership while DRM Hope Pvt
-- Ltd had none. Nothing looked broken - balances still balanced - but the
-- ledgers accrued against the wrong company, which only shows up when someone
-- runs entity-level statutory accounts.
--
-- The exception is per panel, not per site: one or two corporates (notably
-- WCL) remain in panel with the old partnership, so their patients do belong
-- there. That rule therefore lives on the corporate record rather than being
-- hardcoded against a hospital name.
--
-- Ayushman Nagpur is a separate hospital at Ramdas Peth treating private
-- patients only, and is unaffected.

ALTER TABLE public.corporate
  ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES public.companies(id);

COMMENT ON COLUMN public.corporate.company_id IS
  'Legal entity this panel contracts with. Nearly all are DRM Hope Pvt Ltd; a '
  'few legacy panels such as WCL remain with the Hope Hospitals partnership.';

-- Default every panel to the current operating company, then move the known
-- legacy ones back. Extend this list as the billing team confirms it.
UPDATE public.corporate c
   SET company_id = (SELECT id FROM companies WHERE company_key = 'drm_pvt_ltd')
 WHERE c.company_id IS NULL;

UPDATE public.corporate c
   SET company_id = (SELECT id FROM companies WHERE company_key = 'hope_partnership')
 WHERE upper(btrim(c.name)) LIKE 'WCL%'
    OR upper(btrim(c.name)) LIKE '%WESTERN COALFIELD%';

-- One resolver, used by both triggers and the backfill below, so the rule
-- cannot drift between them.
CREATE OR REPLACE FUNCTION public.resolve_patient_company(
...
DECLARE
  v_company_id UUID;
BEGIN
  -- Ayushman is its own hospital and takes private patients only.
  IF lower(COALESCE(p_hospital_name, '')) = 'ayushman' THEN
    SELECT id INTO v_company_id FROM companies WHERE company_key = 'ayushman_nagpur';
    RETURN v_company_id;
  END IF;

  -- Otherwise the panel decides, because the legacy exceptions are per panel.
  IF COALESCE(btrim(p_corporate), '') <> '' THEN
    SELECT c.company_id INTO v_company_id
      FROM corporate c
     WHERE lower(btrim(c.name)) = lower(btrim(p_corporate))
  ...
  SELECT id INTO v_company_id FROM companies WHERE company_key = 'drm_pvt_ltd';
  RETURN v_company_id;
END;
$$;

-- ------------------------------------------------------------------
-- Patient ledger trigger: same body, company resolved by the helper.
-- ------------------------------------------------------------------
...
BEGIN
  ...
  -- The UID in the name is mandatory: mergedLedgerBalances merges rows by
  -- normalized name, so two patients called "Ramesh Kumar" would collapse into
  -- a single ledger row carrying both their balances.
  v_ledger_name := COALESCE(NULLIF(btrim(NEW.name), ''), 'Unnamed Patient')
                   || ' (' || NEW.patients_id || ')';

  v_company_id := public.resolve_patient_company(NEW.hospital_name, NEW.corporate);
...
-- ------------------------------------------------------------------
-- Re-point the ledgers already created under the wrong entity.
-- ------------------------------------------------------------------
UPDATE chart_of_accounts a
   SET company_id = public.resolve_patient_company(p.hospital_name, p.corporate),
       updated_at = now()
  FROM patients p
 WHERE a.account_code = 'PT' || p.patients_id
   AND a.company_id IS DISTINCT FROM
       public.resolve_patient_company(p.hospital_name, p.corporate);

-- ------------------------------------------------------------------
-- Bill accrual: identical body, company resolved by the helper above.
-- ------------------------------------------------------------------
...
BEGIN
  ...
  -- Only the summary carries a discount figure; the bill total is taken as-is.
  IF v_has_fs THEN
