-- Make edits and deletions of an advance payment reach the ledger.
--
-- create_receipt_voucher_for_payment() has only ever been attached to INSERT,
-- so the ledger learns about a payment once and never hears about it again:
--
--   * edit the amount  -> the voucher keeps the original figure
--   * delete the row   -> the voucher stands alone, crediting 2110 for money
--                         that exists nowhere
--
-- Measured on 25 Jul 2026 across Apr-Jul: 61 orphaned vouchers (Rs 4,01,186,
-- since cancelled) and 135 stale ones (Rs 7,01,050). Real examples had a
-- Rs 20,000 voucher against a payment corrected down to Rs 1,000.
--
-- The fix follows this codebase's own convention for correcting a posted
-- voucher - the Voucher Entry screen alters in place and lets
-- log_voucher_changes() record it - so an edit cancels the stale voucher and
-- posts a fresh correct one, keeping payment and voucher one-to-one.
--
-- That needs a link between the two, which has never existed: the trigger
-- writes NULL into reference_number for advances. It now stores the
-- advance_payment id, the same way it already does for
-- patient_payment_transactions.
--
-- IMPORTANT - this works for payments recorded from here on. Rows created
-- before this migration have no link, so editing one cannot find its voucher.
-- Rather than risk double-posting, an edit to such a row is left alone with a
-- warning; see the safety note in the UPDATE branch below.
...
BEGIN
  ...
  INSERT INTO vouchers (
    ...
    narration, total_amount, patient_id, status, created_by, created_at, updated_at
  ) VALUES (
    v_voucher_id,
    v_voucher_number,
    v_voucher_type_id,
...
-- The INSERT trigger is left exactly as it is. 20260725110000 exists because a
-- hand-run duplicate trigger double-minted 706 vouchers, so nothing here
-- re-attaches it.
--
-- UPDATE is restricted to the columns that actually change a posting, so
-- editing a remark or a package field does not churn the ledger.
DROP TRIGGER IF EXISTS trg_advance_payment_sync_voucher ON public.advance_payment;
CREATE TRIGGER trg_advance_payment_sync_voucher
AFTER UPDATE OF advance_amount, payment_mode, payment_date, bank_account_id,
                bank_account_name, is_refund, status, patient_id
    OR DELETE
ON public.advance_payment
FOR EACH ROW
EXECUTE FUNCTION create_receipt_voucher_for_payment();

CREATE INDEX IF NOT EXISTS idx_vouchers_reference_number
  ON public.vouchers(reference_number);
