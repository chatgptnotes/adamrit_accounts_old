-- Make visit_discounts the authority on discounts.
--
-- The discount approval workflow is enforced in practice: a patient's gate pass
-- cannot be generated until the discount is approved, so for any discharged
-- patient the decision is recorded in visit_discounts by construction.
-- DiscountTab.tsx upserts one row per visit and only an admin can set
-- approval_status to 'approved'.
--
-- So the presence of a row now settles the matter rather than merely being a
-- fallback: approved grants the discount, pending or rejected grants nothing.
-- financial_summary is consulted only where no discount decision exists at all,
-- since its figure is typed by hand and carries no approval.
--
-- Body otherwise identical to 20260726210000.
