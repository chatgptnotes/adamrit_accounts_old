-- Fall back to the bill total when there is no category breakdown.
--
-- 20260726130000 read the per-department split from financial_summary. In
-- production that table holds 5 rows totalling Rs 31,301 against a Rs 1.63
-- crore book, because the Financial Summary card on the Final Bill page is
-- almost never saved. bill_line_items holds Rs 1.29 crore but its category
-- columns are unusable: bill_section_id is not populated, so the money cannot
-- be joined to the 10 section titles that would have named the department.
--
-- So: keep the category split whenever a financial summary exists (it is
-- already proven, and grows as staff adopt the card), and otherwise credit the
-- whole bill to 4160 Hospital Services Income. That trades the per-department
-- P&L for coverage of every bill, which is what makes the receivable real.
--
-- Only the V1 credit side changes. V2 (advance adjustment) and V3 (payer
-- split) are unchanged, as is the reversal-on-rejection branch.
