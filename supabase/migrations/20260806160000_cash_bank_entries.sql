-- One-tap cash/bank entries (Bank & Cash tile).
--
-- The last manual voucher types get a guided path: cash deposited into bank,
-- cash withdrawn, bank charges, bank interest. Each call posts the proper
-- voucher (Contra for movements, Journal for charges/interest) with
-- find-or-created Bank Charges / Bank Interest ledgers.
--
...
BEGIN
  BEGIN
    PERFORM public.post_cash_bank_entry('TRANSFER', 'hope', NULL, 100, NULL, 'verify');
    RAISE EXCEPTION 'verification did not reach the expected guard';
  EXCEPTION WHEN SQLSTATE 'P0001' THEN
    IF SQLERRM NOT LIKE '%Kind must be%' THEN RAISE EXCEPTION 'unexpected guard: %', SQLERRM; END IF;
    RAISE NOTICE 'OK — post_cash_bank_entry guards hold';
  END;
END;
$verify$;
