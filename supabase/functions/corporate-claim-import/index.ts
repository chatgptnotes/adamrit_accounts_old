...
const schemeOf = (value: unknown) => {
  ...
  return id.startsWith('PJ') ? 'PMJAY' : id.startsWith('MJ') ? 'MJPJAY' : 'UNRESOLVED';
};
...
serve(async (request) => {
  ...
  try {
    ...
    const body = await request.json() as { hospitalName?: string; schemeCode?: string; sourceKind?: string; files?: IncomingFile[]; contentFingerprints?: string[] };
    ...
    const fileHashes = await Promise.all(files.map(async (file, index) => suppliedHashes[index] || hash({ type: file.fileType, reportDate: file.reportDate, headers: file.headers, rows: file.rows.map((row) => row.originalValues) })));
    const duplicateChecks = await Promise.all(fileHashes.map((sourceHash) => db.from('corporate_claim_import_files').select('id').eq('source_hash', sourceHash).maybeSingle()));
    const newFiles = files.filter((_, index) => !duplicateChecks[index].data);
    const duplicateFiles = files.length - newFiles.length;
    if (!newFiles.length) return new Response(JSON.stringify({ alreadyImported: true, duplicateFiles, validRows: 0, invalidRows: 0, duplicateRows: files.reduce((total, file) => total + file.rows.length, 0) }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    for (const file of newFiles) {
      ...
      for (const row of file.rows) {
        ...
        if (row.issues.length || !stage || schemeConflict) {
          invalidRows += 1;
          await db.from('corporate_claim_import_rows').insert({ import_file_id: fileRow.id, row_number: row.rowNumber, row_fingerprint: fingerprint, original_values: row.originalValues, normalized_values: row.normalizedValues, detected_stage: stage, parse_errors: row.issues.length ? row.issues : [schemeConflict ? `Program ID belongs to ${resolvedScheme}, not ${schemeCode}.` : 'Government stage could not be determined.'] });
          continue;
        }
        validRows += 1;
        ...
        const { data: foundClaim } = await db.from('corporate_claims').select('id,current_stage,current_source_observed_on,paid_amount').eq('hospital_name', hospitalName).eq('scheme_code', claimScheme).eq('normalized_registration_id', registration).maybeSingle();
        let claim = foundClaim;
        if (!claim) {
          const { data, error } = await db.from('corporate_claims').insert({ hospital_name: hospitalName, scheme_code: claimScheme, government_registration_id: row.originalValues['Registration ID'], normalized_registration_id: registration, government_program_id: row.originalValues['Program ID'], normalized_program_id: normalized.program_id, beneficiary_name: row.originalValues['Beneficiary Name'], current_stage: stage, raw_government_status: row.originalValues['Case Status'] || null, payment_state: stage === 'payment_accomplished' ? (numberOrNull(normalized.paid_amount) || 0) > 0 ? 'received' : 'unknown' : stage === 'payment_initiated' || stage === 'claims_sent_to_bank' || stage === 'pending_with_payer' ? 'pending' : stage === 'rejected' ? 'rejected' : 'not_due', claimed_amount: numberOrNull(normalized.claim_amount), approved_amount: numberOrNull(normalized.approved_amount), paid_amount: numberOrNull(normalized.paid_amount) || 0, current_source_observed_on: file.reportDate, last_seen_at: new Date().toISOString() }).select('id,current_stage,current_source_observed_on,paid_amount').single();
        ...
        const { data: patientCandidates } = await db.from('patients').select('id,registration_id,patients_id').or(`registration_id.eq.${row.originalValues['Registration ID']},patients_id.eq.${row.originalValues['Program ID']}`);
        const candidates = patientCandidates || [];
        if (candidates.length === 1) {
          const patient = candidates[0];
          const { data: visit } = await db.from('visits').select('id').eq('patient_id', patient.id).order('created_at', { ascending: false }).limit(1).maybeSingle();
          await db.from('corporate_claims').update({ matched_patient_id: patient.id, matched_visit_id: visit?.id || null, match_state: 'matched', updated_at: new Date().toISOString() }).eq('id', claim.id);
          await db.from('corporate_claim_matches').insert({ claim_id: claim.id, import_row_id: importRow.id, patient_id: patient.id, visit_id: visit?.id || null, method: 'exact_identifier', confidence: 'high', candidate_evidence: [{ registration_id: row.originalValues['Registration ID'], program_id: row.originalValues['Program ID'] }], is_approved: true, reviewed_by: authData.user.email });
        } else {
          unmatchedRows += 1;
          await db.from('corporate_claim_review_items').insert({ claim_id: claim.id, import_row_id: importRow.id, hospital_name: hospitalName, scheme_code: claimScheme, review_type: resolvedScheme === 'UNRESOLVED' ? 'scheme_resolution' : candidates.length > 1 ? 'ambiguous' : 'unmatched', details: { reason: resolvedScheme === 'UNRESOLVED' ? 'Program ID does not begin with PJ or MJ.' : 'No single exact HMIS identifier match was found.', candidate_count: candidates.length } });
        }
    ...
    return new Response(JSON.stringify({ batchId: batch.id, totalRows, validRows, invalidRows, unmatchedRows, duplicateRows, duplicateFiles, snapshotState }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
